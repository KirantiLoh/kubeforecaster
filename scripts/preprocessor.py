...
# Ensure inputs are 1D numpy arrays
        ...
        # Apply log transformation and Savitzky-Golay filter
        ...
        # Generate lagged features (using ORIGINAL data)
        lagged = []
        for lag in range(1, self.lookback_window + 1):
            # Create shifted arrays with zeros padding
        ...
        # Generate rolling features (using ORIGINAL data)
        rolled = []
        if n_samples > 1:
            # CPU rolling (window=2)
            ...
            # Handle edge case with n_samples <= 1
            rolled = [np.zeros(n_samples) for _ in range(4)]

        # Combine all features (filtered + lagged + rolling)
